...
{
    private $codigo, $nombre, $precio_venta, $id_marca, $id_categoria, $id, $estado;
    ...
    {
        $sql = "SELECT p.*, m.id AS marca, m.nombre AS marca, c.id AS categoria, c.nombre AS categoria FROM repuestos p INNER JOIN marcas m ON p.id_marca = m.id INNER JOIN categorias c ON p.id_categoria = c.id";
        $data = $this->selectAll($sql);
        return $data;
    }
